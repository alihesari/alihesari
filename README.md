Senior Software Engineer • Entrepreneur

Building digital products and helping others do the same.

I’ve worked on everything from high-scale backend systems and distributed architecture to frontend frameworks and mobile apps. I’ve also built businesses, launched startups, and taught others how to do the same. Currently working remotely and developing tools for developers and digital entrepreneurs.

### Projects & Startups

- [ProductivityTrack.com](https://productivitytrack.com) – Life OS for Tech Professionals Who Want Clarity, Energy & Execution, Without Burning Out.
- [MakeV1.com](https://makev1.com) – Helping developers launch the first version of an online business.
- [Toolkit.dev](https://toolkit.dev) – A collection of AI-enhanced developer tools.
- [Pixikit.com](https://pixikit.com) – Tools and components for modern UI design and frontend development.
- [HunterKit.com](https://hunterkit.com) – A toolkit for launching, managing, and growing your Product Hunt presence.
- [TechFarsi.com](https://techfarsi.com) – Technology education in Persian for aspiring software engineers and tech professionals.
- [FintechFarsi.com](https://fintechfarsi.com) – Fintech education, tools, and tutorials in Persian.

### Writing & Content

- Sharing startup playbooks, developer productivity tips, and product building strategies on [AliHesari.com](https://alihesari.com).
- Writing an eBook on building an MVP in days, not months.
- Planning YouTube and Twitter/X content around productivity and software engineering.

### Open Source
- [Laravel Social Auto Posting](https://github.com/alihesaridev/laravel-social-auto-posting) – A Laravel package to auto-publish content to social media platforms (Twitter/X, LinkedIn, and more). Designed for automation in content-driven Laravel projects.

### Contact

- Twitter / X: [@alihesaridev](https://x.com/alihesaridev)  
- LinkedIn: [Ali Hesari](https://www.linkedin.com/in/alihesari)  
- Website: [AliHesari.com](https://alihesari.com)  
- Email: ali [at] alihesari.com
